...
list_of_picture_tags_with_unique_class_key = soup.find_all("picture", class_=re.compile(r"SlideImage_picture__a3aKk"))
highest_resolution_url_array = []
...
for picture_tag in list_of_picture_tags_with_unique_class_key:
    ...
    number_of_resolutions = len(resolutions_array);
    highest_resolution_index = number_of_resolutions - 1;

    # need to split the highest resolution url into the url and the resolution
    highest_resolution_url = resolutions_array[highest_resolution_index].split(' ')[0];
    highest_resolution_url_array.append(highest_resolution_url)

print("Title : " + str(heading_title))
print("Total number of Slides : " + str(len(list_of_picture_tags_with_unique_class_key)))
